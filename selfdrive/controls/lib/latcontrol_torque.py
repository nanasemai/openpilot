...
class LatControlTorque(LatControl):
  ...
  def get_predicted_velocity(self, CS, lat_delay):
    ...
    if model_v2 is None:
      return CS.vEgo
    # model_valid only verifies orientation.x length; verify velocity.x separately
    if len(model_v2.velocity.x) < 2 or len(model_v2.velocity.x) != len(model_v2.orientation.x):
      return CS.vEgo
