...
### 方案 2 详细设计：摩擦衰减曲线按中国限速右移

最终曲线（`FRICTION_INTERP_SPEEDS` = `[1, 5, 15, 20, 30, 34] m/s`，
`FRICTION_INTERP_GAIN` = `[1, 1, 1, 1, 0.9, 0.8]`）：

| 速度 | m/s | gain |
|------|-----|------|
| ≤72 km/h (20 m/s) | 1.000 | 与原始一致 |
| 90 km/h (25) | 0.950 | 轻微衰减 |
| 108 km/h (30) | 0.900 | — |
| 120 km/h (33.3) | 0.818 | 法定最高速 |
| >122 km/h (34+) | 0.800 | 钳制 |

设计依据：压线敏感的中低速连续弯（山路 30–50、城市快速路 60–80 km/h）必须保留
原始摩擦增益，衰减只在高速段（≥100 km/h）实质生效。

### 方案 5 详细设计：降低 JERK_GAIN（连续弯切弯心）

- 现象定位：单弯正常、**连续弯**切弯心（偏内侧）、速度 50–70 km/h。
- 机制：`JERK_GAIN * desired_lateral_jerk` 作为摩擦项输入的一部分
  （`ff += friction_gain * get_friction(error + JERK_GAIN * desired_lateral_jerk, ...)`）。
  连续弯换向瞬间 `desired_lateral_jerk` 大 → 额外转向力矩偏强 → 车头钻弯心；
  单弯进入稳态后 jerk 归零被余量掩盖。
- 改动：`JERK_GAIN` 0.30 → **0.25**（-17%），只影响入弯/换向瞬态；
  恒速段、直行、稳态时 jerk≈0 → 零影响。与方案 1、2 无耦合，单值可回退。
- 验证优先级：连续弯压线的主修复来自本项（而非方案 1），见 §6 仿真数据。
...
### 6.2 仿真验证数据（Toyota RAV4 扭矩控制模型，稳态跟踪）

在让车辆实际跟踪弯道（测量≈期望、err≈0）的稳态仿真下，
对比改动前后前馈值，确认各项改动量级：

| 场景 | 旧前馈 (κ·vEgo²) | 新前馈 (κ·v_pred²) | Δ前馈 | 摩擦gain |
|------|-----------------|-------------------|-------|----------|
| 市区弯 R60 40km/h（恒速） | 2.05 | 2.05 | **0%** | 1.00 |
| 连续弯 R100 60 带刹 | 2.79 | 2.74 | -1.8% | 1.00 |
| 快速路弯 R125 72 带刹 | 3.20 | 3.11 | -3.0% | 1.00 |
| 入弯制动 70→55 km/h | 3.76 | 3.48 | **-7.6%** | 1.00 |
| 高速入弯 100→80 km/h | 4.68 | 4.44 | -5.3% | 0.92 |
| 高速弯 R500 120km/h（恒速） | 2.22 | 2.22 | 0% | 0.82 |

**读法**：
- 方案 1（v_pred）只在**带刹入弯**生效，最大单点下降 -7.6%（中-重刹 70→55），
  恒速弯完全为 0 —— 量级温和且方向正确；
- 方案 2（摩擦右移）只在 ≥100 km/h 段生效，120 km/h 仅 -18%，≤72 km/h 恒为 1.0；
- 方案 5（JERK_GAIN）不体现在恒速稳态（jerk≈0），只在入弯/换向瞬态作用，
  是连续弯压线的主修复项；
- 稳态一致性：各种曲线速度下前馈精确等于 曲率×对应速度²，输出扭矩与横向加速度
  需求线性一致，无异常饱和或符号反转。
...
| 项 | 方法 | 预期 |
|----|------|------|
| 连续弯不切弯心 | **中-重刹连续弯 50–70 km/h**，观察轨迹 | 车头不再钻弯心（JERK_GAIN 主修复） |
| 入弯制动不压内线 | 高速入弯带刹 | 前馈下降生效（最大 -7.6%） |
| 弯中恒速循迹 | 恒速过弯 | 与改动前一致（v_pred≈vEgo） |
| 出弯加速掠过弯 | 弯中加油门 | v_pred>vEgo → 参考略增，仍贴合此物理 |
| 直线/蛇形 | 高速直线 ≥100 km/h | 蛇形抑制保留（摩擦 -18~20%） |
| NNLC 开启 | SP 设置开启 NNLC 复测 | 弯中表现更稳 |

### 6.4 主要调参旋钮

- `JERK_GAIN`：连续弯切弯心仍偏重 → 继续降低（如 0.22）；出弯甩尾/回正慢 → 调回。
